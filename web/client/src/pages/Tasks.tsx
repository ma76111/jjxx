        ...
        <>
              ...
              <div key={task.id} className="card space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-semibold text-gray-800 dark:text-gray-100">
                    #{task.id} {task.bot_name || (lang === 'ar' ? 'مهمة خارجية' : task.bot_name || 'External Task')}
                  </span>
                ...
                <div className="flex gap-3 text-sm text-gray-600 dark:text-gray-400 flex-wrap">
                  <span>{task.task_type === 'paid' ? `💰 ${task.reward_per_user} USDT` : `🔄 ${t('exchange')}`}</span>
                  <span>👥 {task.completed_count}/{task.required_count}</span>
                  <span>📋 {task.proof_type}</span>
                </div>
          ...
          {total > 10 && (
            <div className="flex gap-2 justify-center pt-2">
              <button className="btn-secondary" disabled={page === 1} onClick={() => setPage(p => p - 1)}>{t('prevPage')}</button>
              <span className="py-2 px-3 text-sm dark:text-gray-300">{t('page')} {page}</span>
              <button className="btn-secondary" disabled={tasks.length < 10} onClick={() => setPage(p => p + 1)}>{t('nextPage')}</button>
            </div>
          )}
        </>
      )}
