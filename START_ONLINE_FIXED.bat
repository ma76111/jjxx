...
:: Start Web Client with special flags
echo [4/5] تشغيل الموقع...
start "Web Client" cmd /k "cd web\client && set DANGEROUSLY_DISABLE_HOST_CHECK=true && npm run dev"
...
echo    - لو ظهرت مشكلة host، الموقع يشتغل محلياً
echo.
echo ─────────────────────────────────────────
echo.
pause
